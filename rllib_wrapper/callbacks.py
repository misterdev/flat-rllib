...
def on_train_result(info):
    ...
    print(
    '\r\n==> Iteration {:,} - {:,} episodes -- avg score: {:.0f} -- avg length: {:.0f} -- {:>3.0f}% episodes dones -- {:>3.0f}% agents done\n'.format(
        result['training_iteration'],
        result['episodes_total'],
        custom_metrics['score_mean'],
        result['episode_len_mean'],
        custom_metrics['done_mean'] * 100,
        custom_metrics['agents_done_mean'] * 100
    ))